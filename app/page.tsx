...
import { ArrowRight, DownloadIcon, Github, GithubIcon, LoaderCircle } from 'lucide-react';
...
const HomeDetails = observer(() => {
    ...
    const [isDownloading, setIsDownloading] = useState([false, false, false, false]);
    ...
    return (
        <motion.main className="w-full" initial="noFile" animate={previewUrl ? 'hasFile' : 'noFile'}>
            <motion.video
                src="/wave-loop.mp4"
                autoPlay
                muted
            ...
            />
            <motion.div variants={variants.overlay} className="fixed overlay bg-white/85 backdrop-blur-lg pointer-events-none"></motion.div>
            <div className="absolute md:flex justify-between p-4 hidden w-full text-black">
                ...
                <a href="https://github.com/elevenlabs/elevenlabs-examples/tree/main/examples/sound-effects/video-to-sfx" className="font-mono text-sm mb-1 text-gray-900">
                    <span className="pr-2">This code is open source on GitHub</span>
                    <Github className="inline-block" size={16} />
                </a>
            </div>
            <motion.div className={cn('flex flex-col md:hidden text-black p-4 gap-4', previewUrl && 'hidden')}>
                <a className="font-mono text-sm mb-1 text-gray-900" href="https://elevenlabs.io/docs/api-reference/how-to-use-text-to-sound-effects">
                    <span className="underline pr-2">Texts to Sounds Effects API</span>
                    <span className="pr-2">is now live</span>
                </a>
                <a className="font-mono text-sm mb-1 text-gray-900" href="https://github.com/elevenlabs/elevenlabs-examples/tree/main/examples/sound-effects/video-to-sfx">
                    <span className="pr-2">
                        This code is open source <span className="underline">on GitHub</span>
                    </span>
                </a>
            </motion.div>
            <motion.div
                ...
                {mutations.videoToSfx.isPending && (
                    <motion.div variants={variants.loader} className="w-full center font-mono py-4">
                        <LoadingIndicator />
                    </motion.div>
                )}
                {orchestrator && (
...
const Waveform = observer(({ player, barBgColor = 'bg-gray-800/30' }: { player: AudioPlayer; barBgColor: string }) => {
        ...
        return (
            <motion.button
                ...
                <Button
                    ...
                    {isDownloading ? <LoaderCircle className="animate-spin text-gray-800/50" size={16} /> : <DownloadIcon size={16} className="text-gray-800/50" />}
                </Button>
            </motion.button>
        );
    }
