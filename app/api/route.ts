...
const generateSoundEffect = async (prompt: string, maxDuration: number): Promise<string> => {
    ...
    const response = await fetch('https://api.elevenlabs.io/v1/sound-generation', options);
...
const generateCaptionForImage = async (imagesBase64: string[]): Promise<string> => {
    if (!process.env.OPENAI_API_KEY) {
        throw new Error('No API key');
    }
    const response = await openai.chat.completions.create({
...
export async function POST(request: Request) {
    if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
        ...
        if (!success) {
            return new Response(`You have reached your request limit for the hour of ${HOUR} requests. Please try again in 1 hour`, {
                status: 429,
                headers: {
    ...
    try {
        ...
        await Promise.all([...Array(NUM_SAMPLES)].map(() => generateSoundEffect(caption, duration))).then((results) => {
