...
import { motion, useTransform, useScroll as useFramerMotionScroll, MotionValue, useVelocity, useMotionValueEvent, useSpring, SpringOptions, HTMLMotionProps } from 'framer-motion';
...
export const ScrollSmoothing = ({ progress, position, velocity, children }: ScrollSmoothingProps) => {
...
export const WindowScrollProvider = ({ children }: any) => {
    ...
    const scrollApi = React.useMemo(
        ...
        [container, scrollX, scrollY, scrollXProgress, scrollYProgress, scrollXVelocity, scrollYVelocity]
    );
...
export const ScrollTarget = ({
    ...
    const scrollApi = React.useMemo(
        ...
        [inherited.container, scrollX, scrollY, scrollXProgress, scrollYProgress, inherited.velocity.x, inherited.velocity.y]
    );
...
export const ScrollRange = ({ children, zero = 0, one = 1, ...otherProps }: ScrollRangeProps) => {
    const inherited = useScroll();
    ...
    const scrollApi = React.useMemo(
        ...
        [inherited.container, inherited.position.x, inherited.position.y, adjustedXProgress, adjustedYProgress, inherited.velocity.x, inherited.velocity.y]
    );
