import { HTMLMotionProps, SpringOptions, useSpring, useTransform } from 'framer-motion';
import { useState, useEffect, useRef, useContext, createContext, useMemo, useCallback, ReactNode, useId } from 'react';
...
export const MouseProvider = ({ children, defaultX = -1000, defaultY = -1000 }: { children: ReactNode; defaultX?: number; defaultY?: number }) => {
...
export const MouseFollower = ({ children, alignX = 'center', alignY = 'center', ...otherProps }: MouseFollowerProps) => {
    const mouse = useMouse();
    const transform = useTransform(
        [mouse.position.x, mouse.position.y],
        ([x, y]) => `translateX(${x}px) translateY(${y}px) translateX(${PERCENT_BY_ALIGNMENT[alignX] || alignX}) translateY(${PERCENT_BY_ALIGNMENT[alignY] || alignY})`
    );
...
export const MouseTarget = ({ children, data = {}, ...otherProps }: MouseTargetProps) => {
    const id = useId();
...
export const MouseSmoothing = ({ position, velocity, children }: MouseSmoothingProps) => {
